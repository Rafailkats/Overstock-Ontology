...
def saving(string,name,surname,phone,email,address):
   querya="ASK WHERE { ?id a :"+string+" . ?id :address \""+address+"\"^^xsd:string . ?id :firstName \""+name+"\"^^xsd:string ."
...
def savingTr(string,e1,e2,combo1,combo2):
    ...
    with onto:
      ...
      new_indi=onto["Transaction"](id_name,date=e1,hasEmployee=onto["".join(eid)],hasCustomer=onto["".join(cid)],inItem=items,totalPrice=round(float(price),3))
      tkinter.messagebox.showinfo(string,  "Succesfful Entry!!")
      onto.save("Overstock.owl",format="rdfxml")
...
def searchFunction(m,h,c,r,made,a):
    ...
    for i in results:
        iditem=str(i["id"]).replace("http://test.org/onto.owl#","")
        if (onto[iditem].hasAssemblyNeeded != asse) and (asse!=''):
            continue
